...
public class AIController extends AIHelper implements IAIControllable {
	...
	protected static final KeyBinding keyBackground = new KeyBinding(
			"Background Mining (F8)", Keyboard.KEY_F8, "Command Mod");
	protected static final KeyBinding keyNatural = new KeyBinding(
			"Natural Movement (F7)", Keyboard.KEY_F7, "Command Mod");

	// Background mining
	private boolean backgroundMiningEnabled = false;

	// Natural movement — micro-tremor
	private boolean naturalMovementEnabled = true;
	private final Random naturalRandom = new Random();
	private int naturalTickCounter = 0;
	...
	static {
		...
		ClientRegistry.registerKeyBinding(keyBackground);
		ClientRegistry.registerKeyBinding(keyNatural);
	}
	...
	public void onPlayerTick(ClientTickEvent evt) {
		...
		// Handle F8 background mining toggle
		if (keyBackground.isPressed()) {
			backgroundMiningEnabled = !backgroundMiningEnabled;
			getMinecraft().thePlayer.addChatMessage(
				new net.minecraft.util.ChatComponentText(
					backgroundMiningEnabled
					? "\u00a7a[Minebot] Background \u00a7lATIVADO\u00a7r\u00a7a (F8 desativa)"
					: "\u00a7e[Minebot] Background \u00a7lDESATIVADO\u00a7r\u00a7e (F8 ativa)"));
		}

		// Handle F7 natural movement toggle
		if (keyNatural.isPressed()) {
			naturalMovementEnabled = !naturalMovementEnabled;
			getMinecraft().thePlayer.addChatMessage(
				new net.minecraft.util.ChatComponentText(
					naturalMovementEnabled
					? "\u00a7a[Minebot] Movimento natural \u00a7lATIVADO\u00a7r\u00a7a (F7 desativa)"
					: "\u00a7e[Minebot] Movimento natural \u00a7lDESATIVADO\u00a7r\u00a7e (F7 ativa)"));
		}

		// Background mining: close menu/chat temporarily so the bot can tick
		net.minecraft.client.gui.GuiScreen savedScreen = null;
		if (backgroundMiningEnabled && getMinecraft().currentScreen != null) {
			String screenName = getMinecraft().currentScreen.getClass().getSimpleName();
			boolean isInventory = screenName.contains("Inventory") || screenName.contains("Container");
			if (!isInventory) {
				savedScreen = getMinecraft().currentScreen;
		...
		// Restore screen after background mining tick
		if (savedScreen != null) {
			getMinecraft().currentScreen = savedScreen;
		}

		// Natural movement: add micro-tremor to rotation after each tick
		if (naturalMovementEnabled && getMinecraft().thePlayer != null) {
			naturalTickCounter++;
			if (naturalTickCounter % 2 == 0) {
				float tremorYaw   = (naturalRandom.nextFloat() - 0.5f) * 0.3f;
				float tremorPitch = (naturalRandom.nextFloat() - 0.5f) * 0.2f;
				getMinecraft().thePlayer.rotationYaw   += tremorYaw;
				getMinecraft().thePlayer.rotationPitch += tremorPitch;
				float p = getMinecraft().thePlayer.rotationPitch;
				if (p >  89f) getMinecraft().thePlayer.rotationPitch =  89f;
				if (p < -89f) getMinecraft().thePlayer.rotationPitch = -89f;
			}
		}
