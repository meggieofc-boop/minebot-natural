...
public abstract class AIHelper {
	...
	// --- Human-like movement fields ---
	/** Current smoothed yaw (for easing interpolation) */
	private float smoothYaw = Float.NaN;
	/** Current smoothed pitch (for easing interpolation) */
	private float smoothPitch = Float.NaN;
	/** Accumulated ticks since last action (for random micro-delays) */
	private int actionDelayTicks = 0;
	/** Pending delay ticks before next action executes */
	private int pendingDelayTicks = 0;
	/** Easing factor: lower = smoother/slower rotation (0.08 ~ human speed) */
	private static final float EASING = 0.10f;
	/** Max degrees per tick for yaw (horizontal) — human cap */
	private static final float HUMAN_MAX_YAW = 18f;
	/** Max degrees per tick for pitch (vertical) — human cap */
	private static final float HUMAN_MAX_PITCH = 12f;
	/** Chance per tick of adding a small hesitation delay */
	private static final float HESITATION_CHANCE = 0.04f;
	/** Max random hesitation in ticks */
	private static final int MAX_HESITATION_TICKS = 3;
	...
	private boolean face(double x, double y, double z, float yawInfluence,
		...
		if (d3 >= 2.500000277905201E-7D) {
			...
			final float targetYaw = (float) (Math.atan2(d1, d0) * 180.0D / Math.PI) - 90.0F;
			final float targetPitch = (float) -(Math.atan2(d2,
					Math.sqrt(d0 * d0 + d1 * d1)) * 180.0D / Math.PI);

			// --- Human-like smoothing ---
			// Initialise smooth values on first use
			if (Float.isNaN(smoothYaw)) {
				smoothYaw = rotationYaw;
				smoothPitch = rotationPitch;
			}

			// Ease smooth targets toward the real target
			float yawDiff = closestRotation(targetYaw - smoothYaw);
			float pitchDiff = targetPitch - smoothPitch;

			// Add tiny random imperfection to targets (~±1°)
			float imperfectYaw   = yawDiff   + (rand.nextFloat() - 0.5f) * 1.2f;
			float imperfectPitch = pitchDiff + (rand.nextFloat() - 0.5f) * 0.8f;

			// Ease toward the imperfect target
			smoothYaw   += imperfectYaw   * EASING;
			smoothPitch += imperfectPitch * EASING;

			// Clamp per-tick speed to human limits
			float yawChange   = closestRotation(smoothYaw - rotationYaw);
			float pitchChange = smoothPitch - rotationPitch;

			yawChange   = Math.max(-HUMAN_MAX_YAW,   Math.min(HUMAN_MAX_YAW,   yawChange));
			pitchChange = Math.max(-HUMAN_MAX_PITCH, Math.min(HUMAN_MAX_PITCH, pitchChange));

			if (yawInfluence <= 0 && pitchInfluence <= 0) {
				// only test, do not set
				float finalYawDelta   = closestRotation(targetYaw - rotationYaw);
				float finalPitchDelta = targetPitch - rotationPitch;
				return Math.abs(finalYawDelta) < .5f && Math.abs(finalPitchDelta) < .5f;
			}

			// Apply movement (setAngles expects mouse-delta units: divide by 0.15)
			mc.thePlayer.setAngles(yawChange / 0.15f * yawInfluence,
					-pitchChange / 0.15f * pitchInfluence);
			invalidateObjectMouseOver();

			// Check if we're close enough to the real target
			float remainingYaw   = Math.abs(closestRotation(targetYaw - (rotationYaw + yawChange)));
			float remainingPitch = Math.abs(targetPitch - (rotationPitch + pitchChange));
			boolean done = remainingYaw < 1.0f && remainingPitch < 1.0f;

			LOGGER.trace(MARKER_FACING, "human facing — remaining yaw=" + remainingYaw
					+ " pitch=" + remainingPitch + " done=" + done);

			return done;
		}
		// Reset smooth state when very close
		smoothYaw = Float.NaN;
	...
	/**
	 * Returns true if a random hesitation delay is currently active.
	 * Call this in tick-based actions to add human-like micro-pauses.
	 */
	public boolean isHesitating() {
		...
		actionDelayTicks++;
		if (actionDelayTicks > 5 && rand.nextFloat() < HESITATION_CHANCE) {
			pendingDelayTicks = rand.nextInt(MAX_HESITATION_TICKS) + 1;
			actionDelayTicks = 0;
			return pendingDelayTicks > 0;
		}
		return false;
	}
	...
	public void faceAndDestroy(final BlockPos pos) {
		...
		if (isFacingBlock(pos) && !isHesitating()) {
			selectToolFor(pos);
			overrideAttack();
		}
	}
