...
public class AIController extends AIHelper implements IAIControllable {
	...
	public void onPlayerTick(ClientTickEvent evt) {
		...
		// --- Background-mining: keep ticking even when ESC / a screen is open ---
		// Minecraft normally stops sending ClientTickEvents when paused, but
		// Forge's ClientTickEvent fires regardless of currentScreen. We just
		// need to suppress the vanilla "do nothing while a GUI is open" path.
		// We deliberately allow the bot to keep running; the player can still
		// use /minebot stop to halt it manually.
		// Note: we skip when a container/inventory screen is open to avoid
		// accidentally interacting with GUI slots.
		final net.minecraft.client.gui.GuiScreen screen = getMinecraft().currentScreen;
		if (screen != null) {
			// Allow running through the pause/ESC screen but not inventory GUIs
			boolean isEscOrPauseScreen = screen instanceof net.minecraft.client.gui.GuiIngameMenu;
			if (!isEscOrPauseScreen) {
				// Container or other GUI — skip to avoid conflicts
				return;
			}
			// It's the ESC/pause screen — close it and keep the bot going
